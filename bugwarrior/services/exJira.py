...
class ExJiraIssue(Issue):
    ...
    UDAS = {ISSUE_TYPE: {'type': 'string', 'label': 'Issue Type'}, SUMMARY: {'type': 'string', 'label': 'Jira Summary'},
            URL: {'type': 'string', 'label': 'Jira URL', },
            DESCRIPTION: {'type': 'string', 'label': 'Jira Description', },
            FOREIGN_ID: {'type': 'string', 'label': 'Jira Issue ID'},
            ESTIMATE: {'type': 'numeric', 'label': 'Estimate'}, FIX_VERSION: {'type': 'string', 'label': 'Fix Version'},
            CREATED_AT: {'type': 'date', 'label': 'Created At'}, STATUS: {'type': 'string', 'label': "Jira Status"}, }
    UNIQUE_KEY = (URL,)

    PRIORITY_MAP = {'Highest': 'H', 'High': 'H', 'Medium': 'M', 'Low': 'L', 'Lowest': 'L', 'Trivial': 'L', 'Minor': 'L',
                    'Major': 'M', 'Critical': 'H', 'Blocker': 'H', }

    def to_taskwarrior(self):
        return {'project': self.get_project(), 'priority': self.get_priority(), 'annotations': self.get_annotations(),
                'tags': self.get_tags(), 'due': self.get_due(), 'entry': self.get_entry(),

                self.ISSUE_TYPE: self.get_issue_type(), self.URL: self.get_url(), self.FOREIGN_ID: self.record['key'],
                self.DESCRIPTION: self.record.get('fields', {}).get('description'), self.SUMMARY: self.get_summary(),
                self.ESTIMATE: self.get_estimate(), self.FIX_VERSION: self.get_fix_version(),
    ...
    def __get_sprints(self):
        ...
        sprints = sum([fields.get(key) or [] for key in self.origin['sprint_field_names']], [])
        for sprint in sprints:
    ...
    def get_project(self):
        project = self.record['key'].rsplit('-', 1)[0]
        return project.lower() if self.origin['lower'] else project
    ...
    def get_default_description(self):
        return self.build_default_description(title=self.get_summary(), url=self.get_processed_url(self.get_url()),
                                              number=self.get_number(), cls='issue', )
...
class ExJiraService(IssueService):
    ...
    def __init__(self, *args, **kw):
        ...
        self.jira = JIRA(options={'server': self.config.get('base_uri'), 'rest_api_version': 'latest',
                                  'verify': self.config.get('verify_ssl', default=True, to_type=asbool), }, **auth)
        ...
        if self.import_sprints_as_tags:
            field_names = [field for field in self.jira.fields() if field['name'] == 'Sprint']
    ...
    def get_service_metadata(self):
        return {'url': self.url, 'import_labels_as_tags': self.import_labels_as_tags,
                'import_sprints_as_tags': self.import_sprints_as_tags, 'sprint_field_names': self.sprint_field_names,
                'label_template': self.label_template, 'lower': self.lower, }
    ...
    def annotations(self, issue, issue_obj):
        ...
        return self.build_annotations(((comment.author.name, comment.body) for comment in comments),
